...
zpl_umount_begin(struct super_block *sb)
{
	...
	if (zfsvfs) {
		 ...
		 */
		zfsvfs->z_unmounted = Z_UNMOUNT_BEGIN;
		...
		/*
		 * It's necessary to clear z_unmounted, unless the user wants
		 * the filesystem to remain 'unmounted' for the purpose of
		 * forced pool export.  Retaining this state causes the
		 * filesystem to reject all but what's required to enable it
		 * to remain in the mount table.
		 */
		if (!zfs_forced_export_unmount_enabled)
			zfsvfs->z_unmounted = Z_MOUNTED;
	}
}
