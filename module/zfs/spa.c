...
spa_ld_get_props(spa_t *spa)
{
	...
	if (error == 0) {
		...
		spa_prop_find(spa, ZPOOL_PROP_DEDUP_MAX_ENTRIES,
		    &spa->spa_dedup_max_entries);
		spa->spa_dedup_max_size = spa->spa_dedup_max_entries *
		    ddt_entry_size();
    ...
    nvlist_t *zplprops, dsl_crypto_params_t *dcp)
{
	...
	spa->spa_dedup_max_entries =
	    zpool_prop_default_numeric(ZPOOL_PROP_DEDUP_MAX_ENTRIES);
